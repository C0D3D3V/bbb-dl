"""
Copy of https://github.com/vgalin/html2image
...
class Html2Image:
    ...
    def __init__(
        ...
        self.browser = chrome.ChromeHeadless(
            executable=browser_executable, flags=custom_flags, print_command=verbose, verbose_chrome=verbose_chrome
        )
    ...
    def screenshot_url(self, url, output_file='screenshot.png', size=None):
        ...
        self.browser.screenshot(output_path=self.output_path, output_file=output_file, input=url, size=size)
    ...
    def _extend_save_as_param(save_as, desired_length):
        ...
        save_as.extend([f'{filename}_{i}.{extention}' for i in range(missing_name_count + 1)])
    ...
    def screenshot(
        ...
        other_file = [other_file] if isinstance(other_file, str) else other_file
        url = [url] if isinstance(url, str) else url
        save_as = [save_as] if isinstance(save_as, str) else save_as
        size = [size] if isinstance(size, tuple) else size

        planned_screenshot_count = len(html_str) + len(html_file) + len(other_file) + len(url)
